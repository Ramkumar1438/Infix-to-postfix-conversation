...
sample output:
  ...
  1.  Enter infix expression: K+L-M*N+(O^P)*W/U/V*T+Q

Postfix expression: KL+MN*-OP^W*U/V/T*+Q+

  2.   Enter infix expression: (A+B)*C+(D+E)/F+G

Postfix expression: AB+C*DE+F/+G+

  3.   Enter infix expression: a+b*(c^d-e)^(f+g*h)-i

Postfix expression: abcd^e-fgh*+^*+i-

  4,  Enter infix expression: A+(B*C-(D/E^F)*G)*H
